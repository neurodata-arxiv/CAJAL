classdef OCP < handle
    ...
    methods( Access = public )
        ...
        function tokens = getPublicTokens(this)
            url = sprintf('%socp/ca/public_tokens/',...
                    this.serverLocation);
            this.lastUrl = url;
...
%             end
        ...
        function ids = reserve_ids(this, num_ids)
            ...
            % Build URL
            url = sprintf('%socp/ca/%s/reserve/%d/', ...
    ...
    methods( Access = private )
        ...
        function writeBlockData(this, ramonVol, conflictOption)
            ...
            % Build URL           
            urlStr = sprintf('%socp/ca/%s/hdf5/%d/%d,%d/%d,%d/%d,%d/%s/',...
                            this.serverLocation,...
                            this.annoToken,...
                            ramonVol.resolution,...
        ...
        function url = buildOverlayUrl(this, qObj)
            ...
            switch qObj.slicePlane
                case eOCPSlicePlane.xy
                    url = sprintf('%s%s/%0.1f/%s/%s/%d/%d,%d/%d,%d/%d/',...
                        this.serverLocation,...
                        service,...
                        qObj.overlayAlpha,...
                        token,...
        ...
        function url = buildRAMONUrl(this,qObj)
            ...
            elseif  ~isempty(qObj.resolution) && ...
                    ...
                    ~isempty(qObj.zRange)
                % Add resolution
                url = sprintf('%socp/ca/%s/%s/%s/%d/%d,%d/%d,%d/%d,%d/',...
                    this.serverLocation,...
                    token,...
                    idStr,...
                    option,...
        ...
        function url = buildRAMONIdUrl(this,qObj)
            ...
            if ~isempty(qObj.idListPredicates)
                ...
                % build url
                url = sprintf('%socp/ca/%s/query/%s',...
                    this.serverLocation,...
                    this.annoToken,...
                    predicateStr);
            else
                % no predicates listed so get ids of everything
                url = sprintf('%socp/ca/%s/query/',...
                    this.serverLocation,...
                    this.annoToken);
            end
        ...
        function url = buildXyzVoxelIdUrl(this,qObj)
            ...
            url = sprintf('%socp/ca/%s/id/%d/%d/%d/%d/',...
                this.serverLocation,...
                this.getAnnoToken(),...
                qObj.resolution,...
                qObj.xyzCoord(1),...
        ...
        function url = buildSetFieldURL(this, id, field, value)
            ...
            % Build URL
            url = sprintf('%socp/ca/%s/%d/setField/%s/%s/',...
        ...
        function status = getPropagateStatus(this, token)
            ...
            % Build URL
            url = sprintf('%socp/ca/%s/getPropagate/',...
        ...
        function setPropagateStatus(this, token, status)
            ...
            % Build URL
            url = sprintf('%socp/ca/%s/setPropagate/%d/',...
